...
# Continue processing pending tasks in the queue
...
class JudgeDispatcher(DispatcherBase):
    ...
    def _compute_statistic_info(self, resp_data):
        # Save max time and memory usage across all test cases
    ...
    def judge(self):
        ...
        if resp["err"]:
            ...
            # In ACM mode, if all test cases pass, return AC; otherwise return status of first failed test
            # In OI mode, if all test cases pass, return AC; if all fail, return status of first failed test; otherwise return Partially Accepted
        ...
        # Judging complete; attempt to process remaining tasks in queue
        ...
        with transaction.atomic():
            ...
            if self.contest.rule_type == ContestRuleType.ACM:
                ...
                if problem_id not in contest_problems_status:
                    ...
                    # If already AC, skip directly without updating any counters
                    return
    ...
    def _update_acm_contest_rank(self, rank):
        ...
        # Problem was modified earlier; need to re-fetch
        problem = Problem.objects.select_for_update().get(contest_id=self.contest_id, id=self.problem.id)
